...
export default function KepalaDinasDashboard() {
  ...
  const [showStaffChart, setShowStaffChart] = useState(false);

  useEffect(() => {
    requireRole('kepala_dinas');
    fetchData();
  }, []);
  ...
  async function fetchData() {
    ...
    const dinasId = profileData?.dinas_id;
  ...
  return (
    <div>
      <h2 className="text-xl sm:text-2xl font-bold mb-2 text-gray-900">Selamat Datang, Kepala Dinas</h2>
      <p className="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6 md:mb-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 md:mb-8">
        ...
        <div className="bg-white p-3 sm:p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm text-gray-500">Total Staf</p>
              <p className="text-xl sm:text-2xl md:text-3xl font-bold text-purple-600">{totalStaff}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
        {/* Grafik Laporan per Staf - Accordion */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          <button
            onClick={() => setShowStaffChart(!showStaffChart)}
            className="w-full flex justify-between items-center p-4 sm:p-6 text-gray-700 font-semibold text-sm sm:text-base"
          >
            <span>📊 Laporan per Staf</span>
            <span className="text-xl">{showStaffChart ? '▲' : '▼'}</span>
          </button>
          {showStaffChart && (
            <div className="px-4 sm:px-6 pb-4 sm:pb-6 border-t border-gray-100">
              {staffStats.length > 0 ? (
                ...
                <p className="text-gray-500 text-sm mt-4">Belum ada laporan dari staf.</p>
              )}
            </div>
          )}
        </div>
