...
export type DelegasiPermission = (typeof GUVERNUR_MENU_FEATURES)[number];
...
async function checkGubernur(supabase: ReturnType<typeof createSupabaseClient>) {
  ...
  if (!profile || profile.role !== 'gubernur') {
    return { authorized: false, response: NextResponse.json({ error: 'Hanya Gubernur yang dapat mengelola delegasi' }, { status: 403 }) };
  }
...
export async function GET() {
  try {
    ...
    const auth = await checkGubernur(supabase);
...
export async function POST(request: Request) {
  try {
    ...
    // Validasi: target_user_id bukan gubernur
    ...
    if (targetProfile.role === 'gubernur') {
      return NextResponse.json({ error: 'Tidak dapat mendelegasikan ke Gubernur' }, { status: 400 });
    }
    ...
    const validPermissions = permissions.filter((p: string) =>
      GUVERNUR_MENU_FEATURES.includes(p as DelegasiPermission)
    );
